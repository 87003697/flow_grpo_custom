...
核心流程：
  ...
  Phase 2: Decode → PBR Render → Guidance → Backward（一路反传到 cond_proxy.grad）
  Phase 3: 逐步从 tracker 读取梯度，重算 f_θ 并即时 Backward
...
2. tex_phase1_rollout: Tex Phase 1（Rollout + Tracker）
3. tex_phase2a_decode_render: Tex Phase 2a（Decode + Render）
4. phase2_guidance_and_backward: Phase 2（Guidance + Backward）
5. phase3_rollout_grad_backward: Phase 3（逐步重算 + Backward）
...
def shape_frozen_prepare_no_grad(
    ...
    不渲染 Normal（render_normal=False），只解码 mesh/subs 供后续 Tex decode 使用。
    ...
    with torch.no_grad():
        trellis2_shape_forward(
            ...
            is_training=False,
            render_normal=False,  # 仅获取几何，不渲染 Normal
        )
...
def tex_phase1_rollout(
    ...
    Returns:
        RolloutTracker: 已填充 input_trajectory / output_trajectory / timesteps [/ teacher_trajectory]
    """
    cfg = system.cfg
...
def three_phase_tex_step(
    ...
    编排:
      shape_frozen_prepare (no_grad) → Tex Phase 1 (rollout + tracker)
    ...
    # Tex Phase 1: Rollout no_grad + 记录 proxy 轨迹
        ...
        for batch in train_loader:
            ...
            with accelerator.accumulate(system.tex.model):
                with TrainModeGuard(system.tex.model):
                    tex_log = three_phase_tex_step(state, system, global_step)
