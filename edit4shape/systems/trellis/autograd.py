"""
Trellis 三阶段 Autograd 版 — 显存压缩训练。

本模块实现了基于 TRELLIS 架构的 3D 生成系统，采用三阶段 Autograd 架构降低训练显存峰值。
核心流程：图像条件编码 -> 结构采样 (Dense Sampling) -> 特征采样 (Sparse Sampling) -> 解码 -> 渲染

三阶段架构（按执行顺序，_no_grad / _grad 后缀标注梯度状态）：
...
设计参考：trellis2_distill 分支的 trellis2_shape_autograd_async.py
  P2-no-grad + P2-grad 两遍方案来自 V2 async 版的 _p2_no_grad / _p2_grad 设计。

评估路径仍走 trellis_forward（单阶段，无 autograd 拆分）。

主要组件：
1. TrellisState: 存储生成状态（坐标、特征、相机参数、条件编码等）
2. System: 封装 pipeline、renderer、guidance、optimizer 等核心组件
3. rollout_sparse: 执行稀疏特征的去噪采样过程（支持 tracker 模式）
4. trellis_forward: 共享的前向传播（Dense Sampling → Rollout → Render）—— 评估专用
5. three_phase_step: 三阶段训练步（Phase 1 → 2 → 3）
...
def phase2_decode_render_no_grad(
    ...
    对应 V2 async 版的 PendingMicroBatch._p2_no_grad。
...
def phase2_guidance_backward(
    ...
    对应 V2 async 版的 PendingMicroBatch._p2_wait（获取 rgb_grad）逻辑。
...
def phase2_decode_render_grad(
    ...
    对应 V2 async 版的 PendingMicroBatch._p2_grad。
...
def main(argv) -> None:
    ...
    配置文件示例：
        python -m edit4shape.systems.trellis.autograd --config=configs/trellis.py
        ...
        for batch in train_loader:
            ...
            with accelerator.accumulate(pipe_models['slat_flow_model']):
                ...
                with TrainModeGuard(
                    ...
                    # ★ 三阶段训练步：P0 → P1 → P2(no_grad → guid_bw → grad) → P3
                    train_log = three_phase_step(
                        state, system, cfg,
