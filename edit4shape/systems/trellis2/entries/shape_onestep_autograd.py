"""
Trellis2 Shape 训练系统 — Onestep 单步去噪 + FlowEdit 2D Guidance 版本。
...
训练流程：
  ...
  P4a: decode_render (no_grad) → detached comp_rgb
  P4b: guidance-only backward → rgb_grad
  P4c: decode_render (with grad) + backward(rgb_grad) → v_proxy.grad
  P5: relay → θ.grad

与 shape_autograd.py（VJP 三阶段）的区别：
...
def main(argv) -> None:
    ...
    训练 Shape Flow Model，使用 Onestep 策略（Pretrained Rollout + 单步去噪）。

    配置文件示例：
        python -m edit4shape.systems.trellis2.entries.shape_onestep_autograd \
    ...
    if use_wandb and accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="trellis2-shape-onestep",
            config=dict(cfg),
    ...
    for epoch in range(start_epoch, int(cfg.num_epochs)):
        ...
        for batch in train_loader:
            ...
            with TrainModeGuard(system.shape.model):
                shape_log = onestep_shape_step(state, system, global_step, profiler=profiler)

            accum_count += 1
            if accum_count >= accum_steps:
                sync_grads_and_step(system.shape.model, system.shape.optimizer, grad_clipper, n_accumulated=accum_count)
                accum_count = 0
