...
def _dist_barrier(local_rank: int) -> None:
    """用显式 device tensor 执行 all_reduce barrier，避免 NCCL 'device unknown' 挂起。

    直接调用 dist.barrier() 时，若 init_process_group 未传 device_id，
    NCCL 的 device_ids_ 为空，barrier 会因 GPU 映射不确定而概率性挂起。
    改用在目标 GPU 上的 all_reduce 可绕过该问题。
    """
    import torch.distributed as dist
...
def main(argv) -> None:
    ...
    # NCCL 超时保护：集合操作超过 30 分钟时抛异常而非无声挂起，
    # 确保所有 worker 能快速失败，避免 SIGKILL 遗留共享内存。
    ...
    # 不调用 accelerator.prepare() 时 Accelerate 不会自动 set_device，
    # NCCL 会因 device_ids 为空而挂起（"devices used by this process are currently unknown"）。
