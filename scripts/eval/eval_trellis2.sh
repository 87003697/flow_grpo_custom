...
#
# 功能：加载 pretrained (teacher) 和 finetuned (student) Trellis2 模型，
#       执行 Shape→Tex 双阶段 forward，渲染 PBR 图像，
#       使用 CLIP / DINO 计算与输入图像的相似度。
#
# GPU 需求：
#   - 不加载 Guidance 模型，所有卡均用于 DDP 评估
#   - 每卡显存 ~20-24 GB（pipeline + teacher + CLIP + DINO）
#
# 用法：
#   bash scripts/eval/eval_trellis2.sh <checkpoints_dir 或 具体 checkpoint 路径>
...
INPUT="${1:-logs/trellis_mesh_tex_v-1e0_FlowEdit_ada-1e-4_cfg-4_steps-9_12_tgt-1-1_adan_lr-1e-4_eps-1e-4_8GPU/checkpoints}"
...
for CKPT in "${CKPT_LIST[@]}"; do
    ...
    echo "========================================"
    echo "Trellis2 Teacher/Student 对比评估"
    echo "========================================"
