...
def main(argv) -> None:
    ...
    # ---- 强制使用 Mesh 渲染器 + Normal 渲染 ----
    with cfg.unlocked():
        cfg.renderer.type = "mesh"
        if not cfg.renderer.get("mesh", None):
            cfg.renderer.mesh = ml_collections.ConfigDict()
            cfg.renderer.mesh.near = 1.0
            cfg.renderer.mesh.far = 100.0
            cfg.renderer.mesh.bg_color = [0.5, 0.5, 0.5]
    ...
    out_dir = run_root / "eval_teacher_student_normal" / ckpt_tag
