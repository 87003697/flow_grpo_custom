...
def run_sampling(
    ...
    meshes, latents_seq_flat, step_log_probs_flat, step_kl_flat = pipe.stage2_with_logprob(
        ...
        sparse_structure_sampler_params={"dense_steps": int(cfg.dense_steps)},
...
# ------------------------------
# Trellis 对齐接口（Direct3D‑S2 Stage2）
# ------------------------------
def direct3d_s2_stage2_with_logprob(
    pipeline,
    image_conds: dict | None = None,
    ...
    **kwargs,
    ...
    """与 trellis_stage2_with_logprob 对齐的入口，转调 pipeline.stage2_with_logprob。

    约定：
    - stage1_cond_dict 必须包含 'image_path'（str）。
    - slat_sampler_params: 可含 'sigma_min'/'rescale_t'/'mc_threshold'/'use_sde'。
    - sparse_structure_sampler_params: 可含 'dense_steps' 或 'num_inference_steps'（用于 dense 阶段）。
    
    参考：
    - `_reference_codes/Direct3D-S2/direct3d_s2/pipeline.py:253-291, 320-341`（核心稀疏采样与解码）
    - 无同名函数（此为脚本层封装）。
    """
    return pipeline.stage2_with_logprob(
