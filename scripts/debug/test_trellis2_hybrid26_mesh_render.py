...
对比 Hybrid26NormalRenderer（26-neighbor + 几何法向量校正）与 MeshRenderer 的渲染结果。
"""
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="./outputs/hybrid26_comparison")
    ...
    h, subs = decoder.forward_chunked(shape_slat, chunk_size=32, return_subs=True)
    ...
    # 使用 h.coords（mesh 顶点是从 h 来的，不是 subs[-1]）
    ...
    # Hybrid26 渲染器
    hybrid26_renderer = Hybrid26NormalRenderer(rendering_options={
        "resolution": 1024, "ssaa": 1, "near": 1.0, "far": 100.0
    }, device=device)
    ...
    # =========================================================================
    print("\n" + "="*60)
    print("渲染 Hybrid26 Normal")
    print("="*60)
    
    voxel_resolution = 1024
    
    with torch.no_grad():
        hybrid26_render = hybrid26_renderer.render(
            mesh=mesh,
            subs=subs,
            coords=voxel_coords,
            extrinsics=extr[0],
    ...
    hybrid26_normal = hybrid26_render["normal"]  # (H, W, 3)
    hybrid26_mask = hybrid26_render["mask"]  # (H, W)
    print(f"hybrid26_normal: {hybrid26_normal.shape}, mask sum: {hybrid26_mask.sum().item():.0f}")
    
    # 转换为 (3, H, W) 格式以便对比
    hybrid26_normal_chw = hybrid26_normal.permute(2, 0, 1)  # (3, H, W)
    ...
    # Mesh Normal 已经是 [0, 1] 范围
    # Hybrid26 Normal 已经转换为 [0, 1] 范围（在渲染器中）
    print("\n[Mesh vs Hybrid26]（范围均为 [0, 1]）")
    compare_tensors("normal_mesh_vs_hybrid26", mesh_normal, hybrid26_normal_chw, atol=0.1)
    compare_tensors("mask_mesh_vs_hybrid26", mesh_mask, hybrid26_mask, atol=1e-3)
    ...
    # 保存 Hybrid26 渲染结果
    save_normal_image(hybrid26_normal_chw, hybrid26_mask, f"{args.save_dir}/normal_hybrid26.png", is_01_range=True)
    ...
    save_diff_image(mesh_normal, hybrid26_normal_chw, f"{args.save_dir}/diff_mesh_vs_hybrid26.png")
    ...
    for yaw_i in yaw_angles:
        ...
        # Hybrid26 渲染
        with torch.no_grad():
            hybrid26_out = hybrid26_renderer.render(
                mesh=mesh,
                subs=subs,
                coords=voxel_coords,
                extrinsics=extr_i[0],
        ...
        hybrid26_n = hybrid26_out["normal"].permute(2, 0, 1)  # (3, H, W)
        
        compare_tensors(f"yaw{yaw_i}_mesh_vs_hybrid26", mesh_n, hybrid26_n, atol=0.15)
    ...
    for yaw_i in yaw_angles:
        ...
        hybrid26_n = hybrid26_out["normal"].permute(2, 0, 1).detach().cpu()  # (3, H, W)
        hybrid26_m = hybrid26_out["mask"].detach().cpu()  # (H, W)
        if hybrid26_m.dim() == 2:
            hybrid26_m = hybrid26_m.unsqueeze(0)
        hybrid26_m = hybrid26_m.expand_as(hybrid26_n)
        hybrid26_vis = hybrid26_n * hybrid26_m + (1 - hybrid26_m)
        hybrid26_img = (hybrid26_vis.permute(1, 2, 0).numpy() * 255).clip(0, 255).astype(np.uint8)
        hybrid26_views.append(Image.fromarray(hybrid26_img).resize((256, 256)))
    
    # 拼接：上面一行 mesh，下面一行 hybrid26
    grid_width = 256 * len(yaw_angles)
    grid_height = 256 * 2
    grid_img = Image.new('RGB', (grid_width, grid_height), (255, 255, 255))
    
    for i, (mv, hv) in enumerate(zip(mesh_views, hybrid26_views)):
        grid_img.paste(mv, (i * 256, 0))
        grid_img.paste(hv, (i * 256, 256))
    ...
    print("  - normal_hybrid26.png: Hybrid26NormalRenderer 渲染结果")
    print("  - diff_mesh_vs_hybrid26.png: 差异图")
    print("  - multiview_comparison.png: 多视角对比（上 Mesh，下 Hybrid26）")
