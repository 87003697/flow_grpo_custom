"""
TRELLIS.2 26-Neighbor Normal vs Mesh Normal 对比测试

对比基于 26 邻居缺失方向的 voxel 法向量渲染与 mesh 渲染结果。
"""
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="./outputs/neighbor26_comparison")
    ...
    from edit4shape.renderers.diff_voxel_normal_neighbor26 import (
        RenderConfig, render_normal_26neighbor, render_normal_sub, render_normal_sub_hierarchy
    )
    ...
    # =========================================================================
    # 渲染 26-Neighbor Voxel Normal（使用最高分辨率 sub）
    # =========================================================================
    print("\n" + "="*60)
    print("渲染 26-Neighbor Voxel Normal")
    print("="*60)
    
    # 使用最高分辨率的 sub（最后一层）
    highest_sub = subs[-1]
    highest_resolution = 2 ** (len(subs) + 5)  # 64 -> 128 -> 256 -> 512 -> 1024
    print(f"使用 sub[-1]，分辨率={highest_resolution}")
    ...
    # 使用 render_normal_sub 接口
    voxel_normal, voxel_mask = render_normal_sub(highest_sub, config, target_size=(1024, 1024))
    ...
    for i, sub in enumerate(subs):
        layer_resolution = 2 ** (i + 6)  # 64 -> 128 -> 256 -> 512
        layer_config = RenderConfig(
            extrinsic=extr[0],
            intrinsic=intr[0],
            resolution=layer_resolution,
        )
        layer_normal, layer_mask = render_normal_sub(sub, layer_config, target_size=(1024, 1024))
        layer_normals.append(layer_normal.permute(2, 0, 1))  # (3, H, W)
        layer_masks.append(layer_mask.unsqueeze(0).float())  # (1, H, W)
        print(f"  layer[{i}] (res={layer_resolution}): mask sum={layer_mask.sum().item():.0f}")
    
    # =========================================================================
    # 渲染层级软权重 26-Neighbor Normal（可微版本）
    # =========================================================================
    print("\n" + "="*60)
    print("渲染层级软权重 26-Neighbor Normal（可微版本）")
    print("="*60)
    ...
    with torch.no_grad():
        hierarchy_normal, hierarchy_mask = render_normal_sub_hierarchy(subs, config, target_size=(1024, 1024))
    hierarchy_normal_chw = hierarchy_normal.permute(2, 0, 1)  # (3, H, W)
    hierarchy_mask_chw = hierarchy_mask.unsqueeze(0).float()  # (1, H, W)
    print(f"hierarchy_normal: {hierarchy_normal_chw.shape}, mask sum: {hierarchy_mask_chw.sum().item():.0f}")
    ...
    print("\n[Mesh vs 26-Neighbor Voxel]（范围已统一到 [-1, 1]）")
    ...
    print("\n[各层 Sub 与 Mesh 对比]")
    for i, (ln, lm) in enumerate(zip(layer_normals, layer_masks)):
        compare_tensors(f"layer[{i}]_vs_mesh", mesh_normal_neg11, ln, atol=0.1)
    
    print("\n[Hierarchy（可微）vs Mesh]")
    compare_tensors("hierarchy_vs_mesh", mesh_normal_neg11, hierarchy_normal_chw, atol=0.1)
    
    print("\n[Hierarchy vs 单层最高分辨率]")
    compare_tensors("hierarchy_vs_single_layer", voxel_normal_chw, hierarchy_normal_chw, atol=0.1)
    ...
    # 保存 26-Neighbor Voxel 渲染结果
    save_normal_image(voxel_normal_chw, voxel_mask_chw, f"{args.save_dir}/normal_voxel_26neighbor.png", is_normalized=False)
    
    # 保存各层结果
    for i, (ln, lm) in enumerate(zip(layer_normals, layer_masks)):
        save_normal_image(ln, lm, f"{args.save_dir}/normal_voxel_layer{i}.png", is_normalized=False)
    
    # 保存 Hierarchy 渲染结果
    save_normal_image(hierarchy_normal_chw, hierarchy_mask_chw, f"{args.save_dir}/normal_voxel_hierarchy.png", is_normalized=False)
    ...
    # Mesh Normal 转为 [-1, 1] 范围以便对比
    mesh_normal_neg11 = mesh_normal * 2 - 1  # [0, 1] -> [-1, 1]
    save_diff_image(mesh_normal_neg11, voxel_normal_chw, f"{args.save_dir}/diff_mesh_vs_voxel.png")
    
    for i, ln in enumerate(layer_normals):
        save_diff_image(mesh_normal_neg11, ln, f"{args.save_dir}/diff_mesh_vs_layer{i}.png")
    
    # Hierarchy 差异图
    save_diff_image(mesh_normal_neg11, hierarchy_normal_chw, f"{args.save_dir}/diff_mesh_vs_hierarchy.png")
    save_diff_image(voxel_normal_chw, hierarchy_normal_chw, f"{args.save_dir}/diff_single_vs_hierarchy.png")
    
    # =========================================================================
    # 验证 Hierarchy 版本的梯度流（跳过，因为显存不足）
    # =========================================================================
    print("\n" + "="*60)
    print("验证 Hierarchy 版本的梯度流（跳过，显存不足）")
    print("="*60)
    print("  跳过梯度验证以节省显存")
    ...
    for yaw_i in yaw_angles:
        ...
        voxel_n, _ = render_normal_sub(highest_sub, config_i, target_size=(1024, 1024))
    ...
    for yaw_i in yaw_angles:
        ...
        voxel_n, voxel_m = render_normal_sub(highest_sub, config_i, target_size=(1024, 1024))
    ...
    print("  - normal_voxel_26neighbor.png: 26-Neighbor Voxel 渲染结果（单层）")
    print("  - normal_voxel_hierarchy.png: 层级软权重渲染结果（可微）")
    print("  - normal_voxel_layer*.png: 各层 Sub 渲染结果")
    print("  - diff_mesh_vs_voxel.png: Mesh vs Voxel 差异图")
    print("  - diff_mesh_vs_hierarchy.png: Mesh vs Hierarchy 差异图")
    print("  - diff_single_vs_hierarchy.png: 单层 vs Hierarchy 差异图")
    print("  - diff_mesh_vs_layer*.png: Mesh vs 各层差异图")
