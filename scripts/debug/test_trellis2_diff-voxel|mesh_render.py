...
def main():
    ...
    extr_0 = extr[0]  # (4, 4)
    intr_0 = intr[0]  # (3, 3)
    grid_size = torch.tensor([1024, 1024, 1024], device=device, dtype=torch.int)  # (3,)
    origin = torch.tensor([-0.5, -0.5, -0.5], device=device, dtype=dual_vertices.dtype)  # (3,)
    config = RenderConfig(
        intrinsics=intr_0,  # (3, 3)
        extrinsics=extr_0,  # (4, 4)
        resolution=1024,
        voxel_size=1.0 / 1024,
        origin=origin,  # (3,)
