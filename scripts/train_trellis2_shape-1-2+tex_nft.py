...
class TrellisSample:
    ...
    Cascade 模式下额外保存：
    - x0_shape_512_sparse: 低分辨率形状 latent（用于训练 512 模型）
    - cond_patches_512: 低分辨率图像条件（512 分辨率的 DINO 特征）
    """
    x0_shape_sparse: SparseTensor      # 形状 3D 稀疏 latent（高分辨率 1024），形状: (N_points_hr, C_shape)
    x0_tex_sparse: SparseTensor        # 纹理 3D 稀疏 latent，形状: (N_points_hr, C_tex)
    cond_patches: torch.Tensor         # 图像条件 patch embeddings（1024 分辨率），形状: (1, P, C)
    ...
    # === Cascade 模式新增字段 ===
    x0_shape_512_sparse: Optional[SparseTensor] = None  # 低分辨率形状 latent（512），形状: (N_points_lr, C_shape)
    cond_patches_512: Optional[torch.Tensor] = None     # 低分辨率图像条件（512 分辨率），形状: (1, P, C)
...
class TrellisSampleCollection:
    ...
    def move_batch_samples(
        ...
        Returns:
            (cond_batched, x0_shape_batch, x0_tex_batch, routing_probs, 
             x0_shape_512_batch, cond_512_batched):
                - cond_batched: 条件 embeddings（1024），形状 (B, P, C)
                - x0_shape_batch: 批量形状稀疏 latent（1024）
                - x0_tex_batch: 批量纹理稀疏 latent
                - routing_probs: 路由权重 [0,1]，用于正负样本加权
                - x0_shape_512_batch: 批量形状稀疏 latent（512）
                - cond_512_batched: 条件 embeddings（512）
        """
        # 拼接条件 embeddings（1024 分辨率）
        cond_batched = torch.cat([s.cond_patches.to(device=device, dtype=dtype) for s in batch_samples], dim=0)
        # 合并形状稀疏 latent（1024 分辨率）
        ...
        # 合并低分辨率形状 latent（512）
        shape_512_list = [s.x0_shape_512_sparse.to(device=device, dtype=dtype) for s in batch_samples]
        x0_shape_512_batch = prepare_sparse_tensor_batch(shape_512_list, batch_size=len(batch_samples))
        # 拼接低分辨率条件（512）
        cond_512_batched = torch.cat([s.cond_patches_512.to(device=device, dtype=dtype) for s in batch_samples], dim=0)
        
        return cond_batched, x0_shape_batch, x0_tex_batch, routing_probs, x0_shape_512_batch, cond_512_batched
    ...
    def build_samples_from_generation(
        ...
        k: int,
        # === Cascade 模式新增参数 ===
        all_shape_512_latents: Optional[List[SparseTensor]] = None,
        cond_batch_512: Optional[torch.Tensor] = None,
    ) -> List[TrellisSample]:
        ...
        Args:
            ...
            all_shape_latents: 对应的形状稀疏 latent 列表（高分辨率 1024）
            all_tex_latents: 对应的纹理稀疏 latent 列表
            cond_batch: 条件 embeddings（1024 分辨率），形状 (B, P, C)
            rewards: 平均奖励列表，长度 BK
            reward_parts_local: 各奖励组件的分数字典
            batch_meta: 图像元数据列表，长度 B
            batch_paths: 图像路径列表，长度 B
            k: 每张图的候选数
            all_shape_512_latents: 可选，低分辨率形状 latent 列表（512）
            cond_batch_512: 可选，低分辨率条件（512 分辨率），形状 (B, P, C)
        ...
        for s in range(BK):
            ...
            # 处理低分辨率 latent 和条件（Cascade 模式）
            shape_512_src = all_shape_512_latents[s] if s < len(all_shape_512_latents) else all_shape_512_latents[-1]
            shape_512_coords_cpu = shape_512_src.coords.clone().detach().cpu()
            shape_512_coords_cpu[:, 0] = 0
            shape_512_feats_cpu = shape_512_src.feats.detach().cpu()
            shape_512_cpu = SparseTensor(
                feats=shape_512_feats_cpu, 
                coords=shape_512_coords_cpu, 
                layout=[slice(0, shape_512_feats_cpu.shape[0])]
            )
            cond_512_s = cond_batch_512[s // k:s // k + 1].detach().cpu()
            ...
            sample = TrellisSample(
                ...
                # === Cascade 模式新增字段 ===
                x0_shape_512_sparse=shape_512_cpu,
                cond_patches_512=cond_512_s,
            )
    ...
    with EvalModeGuard(dense_eval_module, sparse_eval_module):
        for eval_batch in tqdm(
            ...
            with torch.inference_mode():  # 关闭梯度追踪，节省显存
                ...
                # ============================================================
                _eval_ss_res = int(config.resolution.ss)
                _eval_hr_res = int(config.resolution.hr)
                
                B = int(cond_batch.shape[0])
                coords_all, _ = pipeline.stage_1(
                    cond={"cond": cond_batch, "neg_cond": neg_filled},
                    ss_resolution=_eval_ss_res,       # 从配置读取
                    num_samples=B,
                )  # (N_total, 4) - 所有样本的坐标
                ...
                for b, sl in enumerate(shape_slat_batched.layout):
                    ...
                    # 解码为 mesh
                    mesh_obj = pipeline.export_mesh(sample_slat, tex_slat=tex_slat, resolution=_eval_hr_res)
...
def main(_):
    ...
    # =========================================================================
    # 第四部分：训练模型准备（Cascade 模式：同时训练 512/1024 shape 和 tex 模型）
    # =========================================================================
    # 
    # Trellis2 的 Stage2 在 Cascade 模式下包含三个 SLatFlowModel：
    # - shape_slat_flow_model_512: 低分辨率形状 latent (C=32)
    # - shape_slat_flow_model_1024: 高分辨率形状 latent (C=32)
    # - tex_slat_flow_model_1024: 纹理 latent (C=32)，依赖 shape_norm 作为条件
    # 
    # 本脚本同时训练所有模型，使用独立的优化器和分阶段反向传播策略。
    # =========================================================================
    
    # 获取 512 Shape 模型并应用 LoRA
    shape_model_512 = pipeline.models.get("shape_slat_flow_model_512")
    if shape_model_512 is None:
        raise ValueError("Cascade 模式需要 shape_slat_flow_model_512，请检查预训练权重")
    shape_model_512 = apply_lora_if_needed(shape_model_512, config)
    
    # 获取高分辨率 Shape 模型并应用 LoRA
    hr_resolution = int(config.resolution.hr)
    lr_resolution = int(config.resolution.lr)
    shape_model = pipeline.get_shape_flow_model(resolution=hr_resolution)
    ...
    # 获取可训练参数（LoRA 层的参数）
    shape_512_trainable_params = [p for p in shape_model_512.parameters() if p.requires_grad]
    ...
    all_trainable_params = shape_512_trainable_params + shape_trainable_params + tex_trainable_params
    
    # 构建三个独立的优化器
    optimizer_shape_512 = build_optimizer(shape_512_trainable_params, config)
    ...
    shape_model_512, shape_model, tex_model, optimizer_shape_512, optimizer_shape, optimizer_tex = accelerator.prepare(
        shape_model_512, shape_model, tex_model, optimizer_shape_512, optimizer_shape, optimizer_tex
    )
    
    # 设置 LoRA adapter 和梯度检查点
    set_model_adapter(shape_model_512, "default")
    set_model_adapter(shape_model, "default")
    set_model_adapter(tex_model, "default")
    enable_gradient_checkpointing_if_needed(shape_model_512, accelerator, config)
    ...
    pipeline.set_shape_flow_model(shape_model_512, resolution=lr_resolution)
    pipeline.set_shape_flow_model(shape_model, resolution=hr_resolution)
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        for batch_idx, (batch_images, batch_paths, batch_meta) in enumerate(tqdm(loader_iter, disable=not accelerator.is_main_process)):
            ...
            with EvalModeGuard(shape_model_512, shape_model, tex_model):
                with torch.inference_mode():
                    ...
                    # ============================================================
                    # Cascade 模式：低分辨率 → 高分辨率级联采样
                    # ============================================================
                    # 从配置读取分辨率参数
                    _ss_res = int(config.resolution.ss)
                    _lr_res = int(config.resolution.lr)
                    _hr_res = int(config.resolution.hr)
                    _max_tokens = int(config.resolution.max_tokens)
                    
                    # Step 1: 准备两个分辨率的图像条件
                    cond_batch_512 = pipeline.prepare_image_conditions(batch_images, resolution=_lr_res)  # (B, P, C)
                    cond_batch_1024 = pipeline.prepare_image_conditions(batch_images, resolution=_hr_res)  # (B, P, C)
                    
                    # 将条件扩展到 BK 个候选（每图 K 个）
                    cond_512_bk = cond_batch_512.repeat_interleave(k, dim=0)  # (BK, P, C)
                    cond_1024_bk = cond_batch_1024.repeat_interleave(k, dim=0)  # (BK, P, C)
                    BK = cond_512_bk.shape[0]  # 总候选数

                    # Step 2: Stage1 - 生成稀疏结构坐标
                    coords_all, _ = pipeline.stage_1(
                        cond={
                            "cond": cond_512_bk,
                            "neg_cond": torch.zeros_like(cond_512_bk),
                        },
                        ss_resolution=_ss_res,  # 从配置读取
                        num_samples=BK,
                    )  # coords_all: (N_total, 4)

                    # Step 3: Cascade Shape 采样（低分辨率 → 高分辨率）
                    shape_slat_batched, shape_slat_512_batched, actual_resolution = pipeline.stage_2_shape_cascade(
                        lr_cond={
                            "cond": cond_512_bk,
                            "neg_cond": torch.zeros_like(cond_512_bk),
                        },
                        hr_cond={
                            "cond": cond_1024_bk,
                            "neg_cond": torch.zeros_like(cond_1024_bk),
                        },
                        coords=coords_all,
                        lr_resolution=_lr_res,
                        hr_resolution=_hr_res,
                        max_num_tokens=_max_tokens,
                    )  # shape_slat_batched: SparseTensor(N_hr, C), shape_slat_512_batched: SparseTensor(N_lr, C)

                    # Step 3.5: 批量纹理 latent 生成（使用高分辨率条件和 shape）
                    tex_slat_batched, _ = pipeline.stage_2_tex(
                        cond={
                            "cond": cond_1024_bk,
                            "neg_cond": torch.zeros_like(cond_1024_bk),
                    ...
                    all_shape_512_latents = shape_slat_512_list
                    all_tex_latents = tex_slat_list
                    # 保存条件用于训练
                    cond_batch = cond_batch_1024
                    cond_batch_512_for_train = cond_batch_512
            ...
            all_samples.extend(
                TrellisSampleCollection.build_samples_from_generation(
                    ...
                    k=k,
                    # === Cascade 模式参数 ===
                    all_shape_512_latents=all_shape_512_latents,
                    cond_batch_512=cond_batch_512_for_train,
                ),
            )
        ...
        # 计算本 epoch 使用的时间步索引（均匀采样）
        frac = float(config.train.timestep_fraction)
        used_sparse = compute_timestep_usage(steps_sparse, frac)
        train_step_indices_sparse = np.linspace(0, steps_sparse - 1, used_sparse, dtype=np.int32)
        ...
        for inner_epoch in range(int(config.train.num_inner_epochs)):
            ...
            for batch_idx, batch_samples in enumerate(batch_iter):
                # 将样本移动到 GPU 并准备训练数据（包含 512 和 1024 模型的数据）
                (cond_batched, x0_shape_batch, x0_tex_batch, routing_probs, 
                 x0_shape_512_batch, cond_512_batched) = TrellisSampleCollection.move_batch_samples(
                ...
                for j in step_iter:
                    ...
                    t_norm_value = t_value / 1000.0  # 归一化到 [0, 1]
                    t = torch.full((batch_size,), t_value, device=accelerator.device, dtype=torch.float32)  # (B,)

                    # =================================================================
                    # 阶段 0: Shape 512 模型训练（低分辨率阶段）
                    # =================================================================
                    # Flow Matching 前向过程（Shape 512）
                    num_points_512 = x0_shape_512_batch.feats.shape[0]  # 512 总点数
                    t_norm_per_point_512 = torch.full((num_points_512, 1), t_norm_value, 
                                                     device=x0_shape_512_batch.feats.device, 
                                                     dtype=x0_shape_512_batch.feats.dtype)  # (N_512, 1)
                    noise_feats_512 = torch.randn_like(x0_shape_512_batch.feats)  # (N_512, C_shape)
                    xt_feats_512 = x0_shape_512_batch.feats * (1.0 - t_norm_per_point_512) + noise_feats_512 * t_norm_per_point_512
                    xt_512_sparse = sparse_clone_with_feats(x0_shape_512_batch, xt_feats_512)
                    
                    # Shape 512 模型前向传播（使用 512 分辨率的条件）
                    with accelerator.autocast():
                        shape_512_output = shape_model_512(xt_512_sparse, t, cond_512_batched, None, guidance_scale=1.0)
                    
                    # Shape 512 KL 正则化
                    if kl_beta > 0.0:
                        with torch.no_grad():
                            unwrapped_512 = accelerator.unwrap_model(shape_model_512)
                            with unwrapped_512.disable_adapter():
                                shape_512_output_ref = unwrapped_512(xt_512_sparse, t, cond_512_batched, None, guidance_scale=1.0)
                    else:
                        shape_512_output_ref = None
                    
                    # Shape 512 DiffusionNFT 损失计算
                    t_norm_full_512 = torch.full_like(xt_512_sparse.feats, t_norm_value)
                    shape_512_ref_feats = shape_512_output_ref.feats if shape_512_output_ref is not None else shape_512_output.feats
                    shape_512_positive_feats = nft_beta * shape_512_output.feats + (1.0 - nft_beta) * shape_512_ref_feats
                    shape_512_negative_feats = (1.0 + nft_beta) * shape_512_ref_feats - nft_beta * shape_512_output.feats
                    shape_512_positive_sparse = sparse_clone_with_feats(shape_512_output, shape_512_positive_feats)
                    shape_512_negative_sparse = sparse_clone_with_feats(shape_512_output, shape_512_negative_feats)
                    x0_512_pos_feats = xt_512_sparse.feats - shape_512_positive_sparse.feats * t_norm_full_512
                    x0_512_neg_feats = xt_512_sparse.feats - shape_512_negative_sparse.feats * t_norm_full_512
                    x0_512_pos = sparse_clone_with_feats(xt_512_sparse, x0_512_pos_feats)
                    x0_512_neg = sparse_clone_with_feats(xt_512_sparse, x0_512_neg_feats)
                    
                    policy_512 = TrellisSampleCollection.compute_sparse_policy_loss(
                        batch_samples=batch_samples,
                        x0_pos=x0_512_pos,
                        x0_neg=x0_512_neg,
                        x0_ref=x0_shape_512_batch,
                        routing_probs=routing_probs,
                        nft_beta=nft_beta,
                        mode="self",
                    )
                    shape_512_policy_loss = (policy_512.policy_vec * adv_clip_max).mean()
                    shape_512_kl_loss = sparse_batch_mse(shape_512_output, shape_512_output_ref).mean() if shape_512_output_ref is not None else torch.zeros(1, device=accelerator.device)
                    
                    # Shape 512 总损失并反向传播
                    shape_512_total_loss = shape_512_policy_loss + kl_beta * shape_512_kl_loss
                    accelerator.backward(shape_512_total_loss)
                    
                    # Shape 512 优化器更新
                    if accelerator.sync_gradients:
                        if max_grad_norm > 0.0:
                            accelerator.clip_grad_norm_(shape_model_512.parameters(), max_grad_norm)
                        optimizer_shape_512.step()
                        optimizer_shape_512.zero_grad(set_to_none=True)
                    
                    # 保存指标
                    shape_512_pos_mean = policy_512.pos_mean.detach()
                    shape_512_neg_mean = policy_512.neg_mean.detach()
                    shape_512_policy_loss_val = shape_512_policy_loss.detach()
                    shape_512_kl_loss_val = shape_512_kl_loss.detach()
                    
                    # 清理 512 计算图
                    del shape_512_output, shape_512_output_ref, xt_512_sparse
                    del shape_512_positive_sparse, shape_512_negative_sparse
                    del x0_512_pos, x0_512_neg
                    torch.cuda.empty_cache()

                    # =================================================================
                    # 阶段 1: Shape 1024 模型训练（独立反向传播）
                    # =================================================================
                    # 
                    # 显存优化策略：分阶段训练
                    # ----------------------------
                    ...
                    # =================================================================
                    # 合并日志指标（512 + 1024 + tex 三个模型）
                    # =================================================================
                    policy_loss_self = shape_512_policy_loss_val + shape_policy_loss_val + tex_policy_loss.detach()
                    kl_loss = shape_512_kl_loss_val + shape_kl_loss_val + tex_kl_loss.detach()
                    pos_mean = (shape_512_pos_mean + shape_pos_mean + policy_tex.pos_mean.detach()) / 3.0
                    neg_mean = (shape_512_neg_mean + shape_neg_mean + policy_tex.neg_mean.detach()) / 3.0
