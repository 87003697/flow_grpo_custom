...
def trellis_flow_step_with_logprob(
    ...
    Returns:
        Tuple[sp.SparseTensor, torch.Tensor, sp.SparseTensor, torch.Tensor]:
            ...
            - std_dev: 标准差
    """
    # 时间步长与归一化（时间域 Δt = (t - t_prev)/1000 ≥ 0）
    ...
    dt_abs = torch.tensor((t - t_prev) / 1000.0, device=device, dtype=torch.float32)  # 标量 () ≥ 0
    ...
    # SDE：时间域加噪（Euler–Maruyama）：prev = mean + noise_strength * ε
