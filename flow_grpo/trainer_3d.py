"""
Hunyuan3D GRPO Trainer - 优化版本（原simplified版本）

核心优化：
1. 统一入口：在采样阶段确保数据格式统一
2. 简单假设：训练阶段直接假设数据格式正确
3. 保持原生接口：始终使用contexts={'main': cond}
4. 修复维度匹配：timesteps的batch size与其他数据一致
5. 数值稳定性：处理梯度和loss计算中的edge cases
"""
...
class Hunyuan3DGRPOTrainer:
    ...
    def sample_meshes_with_rewards(
        ...
        # 处理timesteps
        timesteps_tensor = self._get_timesteps(len(all_pil_images), num_inference_steps - 1)  # 🔧 修复：使用实际样本数而不是输入图像数
