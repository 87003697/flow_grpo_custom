...
def build_support_for_mesh(mesh: Any, fixed_poses, ref_renderer, device: torch.device, H: int, W: int, camera_param_dim: int, imgs_query: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """为单个 mesh 构建 support 图像与 pose 编码，并拼接 query。

    输入:
        mesh: 原始 mesh 对象。
        fixed_poses: 预设相机位姿集合。
        ref_renderer: 参考渲染器实例。
        device: 设备。
        H, W: 图像尺寸。
        camera_param_dim: 支持 9 编码或 12 展平外参。
        imgs_query: (1,3,H,W) 的 query 张量。
    输出:
        images_seq: (1,S,3,H,W)
        pose_sup: (S-1,D)
...
def build_support_batches(meshes: List[Any], idxs: List[int], imgs_query: torch.Tensor, H: int, W: int, camera_config_py: str, camera_param_dim: int, img_size: int, device: torch.device):
    ...
    supports = []     # 形状: 列表
    for j in idxs:
        images_seq, pose_sup = build_support_for_mesh(meshes[j], fixed_poses, ref_renderer, device, H, W, camera_param_dim, imgs_query)
