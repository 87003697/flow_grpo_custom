...
★ Guidance 配置分两层：
  ...
  - cfg.{stage}.guidance: 运行时配置（prompt, loss 权重等），每次调用 edit 传入
"""
...
def get_default_config(mode: Literal["shape", "tex", "shape_tex"] = "shape_tex"):
    ...
    cfg.contrastive = _build_contrastive()
...
def _build_flowedit_edit_runtime():
    """FlowEdit edit-only 运行时参数（Contrastive 专用）。

    只包含 edit() 路径实际读取的字段：
    - prompt / CFG scales / seed / bg_color — _edit_single 读取
    - loss.src_branch — 决定 use_src_record（Contrastive 不需要 src tracker）

    ★ compute_guidance() 路径额外需要的 reduce_mode / ada_normalize / ada_eps /
      latent loss / pixel loss 字段在此不包含。
    """
    cfg = ml_collections.ConfigDict()
    ...
    # _edit_single 通过 getattr(cfg, 'loss', None) 读取，仅用 src_branch
    cfg.loss = ml_collections.ConfigDict()
    cfg.loss.src_branch = 0.0   # = 0 不启用 src tracker
...
def _build_stage_train():
    ...
    cfg.loss.reg_type = "v"  # 正则化类型: "v" | "x0" | "x1"
...
def _build_shape_stage():
    ...
    # --- Shape Guidance 运行时配置（edit-only，Contrastive 专用） ---
...
def get_config():
    ...
    cfg.run_name = "trellis2_shape_tex_contrastive"
    return cfg
